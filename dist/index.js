...
(() => {
  ...
  // src/js/components/navigation.ts
  function navigationScroll() {
    ...
    if (navigation.getAttribute("data-info-bar") === "true") {
      ...
      infoClose.addEventListener("click", () => {
        ...
        const navigationMobileEl = document.querySelector(
          ".navigation_mobile_wrapper"
        );
        if (navigationMobileEl && navigation.classList.contains("is-open") && window.innerWidth <= 991) {
  ...
  function navigationMobile() {
    ...
    const navigation = document.querySelector(".navigation");
    const navigationMobile2 = document.querySelector(
      ".navigation_mobile_wrapper"
    );
    const infoBar = document.querySelector(".g_info-bar");
    const button = document.querySelector(".navigation_mobile_button");
    if (!navigation || !navigationMobile2 || !infoBar || !button) return;
    button.addEventListener("click", () => {
      ...
      if (!isOpen) {
        navigationMobile2.style.height = "0svh";
        startLenis();
      } else {
        stopLenis();
        if (infoBarClosed || isFixed) {
          navigationMobile2.style.height = "100svh";
        } else {
          navigationMobile2.style.height = "calc(100svh - 2.875rem)";
        }
      }
    });
  }
  ...
  var easeCustom = cubicBezier(0.625, 0.05, 0, 1);
  function setupLenis() {
      ...
      anchor.addEventListener(
        ...
        (e) => {
          ...
          if (url.pathname === window.location.pathname && url.hash) {
            ...
            const targetY = targetEl ? targetEl.getBoundingClientRect().top + window.scrollY : 0;
            const currentY = window.scrollY;
            const distance = Math.abs(targetY - currentY);
            const pxPerSecond = 2e3;
